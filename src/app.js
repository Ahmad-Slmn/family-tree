// app.js — تهيئة وربط الواجهة بدون تعديل السلوك (منظم + مضغوط)
...
import {
  initValidationUI, setValidationResults, getValidationSummary,
  refreshValidationBadge, vcToastSummaryText, clearValidation
} from './ui/validationCenter.js';
import {
  byId, showSuccess, showInfo, showError, showWarning,
  highlight, applySavedTheme, getToastNodes
} from './utils.js';
...
function familiesAreLoaded() {
  return !!window.__familiesLoaded;
}


/** تطبيق الثيم + مزامنة theme-color + تحديث شعار splash */
function applyThemeEverywhere(theme) {
...
const TAG_STATE_KEY = 'treeTaglineState';
...
function startRotatingTagline() {
  ...
  const save = () => {
    localStorage.setItem(TAG_STATE_KEY, JSON.stringify({ index, i, dir, ts: _now() }));
  };
  ...
  const tick = () => {
    ...
    if (dir === 1) {
      ...
      if (--i <= 0) {
        ...
        localStorage.setItem('treeTaglineIndex', index); // توافق قديم
        save();
...
/* =========================
   2) Splash + قياس الأداء
========================= */
let currentSplashProgress = 0;
let splashHasError = false;
...
// أدوات قياس الأداء (اختيارية)
const perf = (() => {
  const marks = new Map();
  const now = () => (performance?.now?.() || Date.now());
  return {
    start(label) { if (!PERF_DEBUG) return; marks.set(label, now()); console.log(`[perf] ▶ ${label}`); },
    end(label) {
...
function setSplashProgress(value, label) {
  ...
  if (bar) bar.style.inlineSize = v + '%';
  if (text) text.textContent = v + '%';
  if (progEl) progEl.setAttribute('aria-valuenow', String(v));

  // تحديث العنوان الفرعي فقط في حالة التحميل الطبيعي
...
function smartSplashMsg(raw) {
  ...
  if (m.includes('timeout') || m.includes('timed out') || m.includes('مهلة') || m.includes('انتهت مهلة')) {
  ...
  if (
    m.includes('indexeddb') || m.includes('quota') || m.includes('quotaexceeded') ||
    m.includes('quota exceeded') || m.includes('storage') || m.includes('securityerror') ||
    (m.includes('transaction') && m.includes('failed'))
  ) {
    return 'تعذر حفظ/قراءة البيانات من التخزين في هذا المتصفح. جرّب نافذة خاصة أو متصفحًا آخر، ثم اضغط "إعادة المحاولة".';
  }

  if (
    m.includes('404') || m.includes('not found') || m.includes('failed to fetch') ||
    m.includes('net::err') || (m.includes('load') && m.includes('resource'))
  ) {
...
function hideSplash(force = false) {
    ...
    const doHide = () => {
        ...
        const finishHide = () => {
            ...
            // إعادة الرسم بعد اختفاء شاشة البداية تمامًا واستقرار تخطيط الواجهة
    ...
    if (wait > 0 && !force) setTimeout(doHide, wait);
    else doHide();
}
...
// Fallback عند load (بدون تغيير السلوك)
window.addEventListener('load', () => {
  if (window.__bootDone) return;
  if (!window.__bootStarted && !splashHasError) armSplashTimeout();
});

/* =========================
   3) Bus (حافلة أحداث بسيطة)
========================= */
const bus = (() => {
  const m = new Map();
...
/* =========================
   5) PIN Lock (UI-only) — نفس المنطق، تنظيم فقط
========================= */
const PIN_KEYS = {
  enabled: 'pin_enabled',
  salt: 'pin_salt',
...
const PIN_SESSION_KEYS = { locked: 'pin_ui_locked' };

function _ssGet(k, def = null) { try { const v = sessionStorage.getItem(k); return v == null ? def : v; } catch { return def; } }
function _ssSet(k, v) { try { sessionStorage.setItem(k, String(v)); } catch { } }

let _pin = {
  el: null, input: null, unlockBtn: null, msg: null, hint: null,
  eyeBtn: null,
  locked: false, intervalId: null, trapHandler: null,
  cooldownTimer: null,
  prevFocus: null
};
...
function _isPinEnabled() { return _lsGet(PIN_KEYS.enabled, '0') === '1' && _hasPinConfigured(); }

function _isForceLocked() { return _lsGet(PIN_KEYS.forceLocked, '0') === '1';}

function _lockReasonMessage() {
  const r = (_lsGet(PIN_KEYS.lockReason, '') || '').trim();

  if (r === 'manual') return '🔒 تم قفل الواجهة يدويًا.';
  if (r === 'visibility') return '🔒 تم قفل الواجهة عند مغادرة التبويب.';
  if (r === 'idle') return '🔒 تم قفل الواجهة بسبب عدم النشاط.';
  if (r === 'idle_away') return '🔒 تم قفل الواجهة بسبب عدم النشاط أثناء مغادرة التبويب.';
  if (r === 'session_end') return '🔒 انتهت مدة الجلسة المفتوحة.';
  return '';
}
...
function initPinLockUI() {
  ...
  _pin.unlockBtn.addEventListener('click', () => verifyAndUnlockFromUI());
  _pin.input.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') { e.preventDefault(); verifyAndUnlockFromUI(); }
  });
...
function unlockUI() {
  ...
  _lsDel(PIN_KEYS.forceLocked);
  _lsDel(PIN_KEYS.lockReason);
...
if (until > now) {
  ...
  try {
    ...
    let cooldownMs = 0;
    if (tries >= 15) cooldownMs = 5 * 60 * 1000;
    else if (tries >= 10) cooldownMs = 2 * 60 * 1000;
    else if (tries >= 6) cooldownMs = 30 * 1000;
    else if (tries >= 3) cooldownMs = 10 * 1000;
...
function _syncTopBarSessionCountdown() {
  ...
  if (!el) { _syncTopBarLockNowBtn(); return; }
...
function _syncPinSessionLabelCountdown() {
  ...
  if (title) title.textContent = open ? 'الجلسة المفتوحة مفعّلة' : 'مدة الجلسة المفتوحة';
...
function _startTopBarSessionCountdown() {
  ...
  __pinSessionCountdownTimer = setInterval(async () => {
    ...
    var u = parseInt(_lsGet(PIN_KEYS.sessionUntil, '0'), 10) || 0;
    if (u && u <= _now()) {
...
document.addEventListener('visibilitychange', async () => {
  ...
  if ('BroadcastChannel' in window) {
    ...
    bc.addEventListener('message', (e) => {
      ...
      if (key === PIN_KEYS.enabled) {
        ...
        if (v === '0') { unlockUI(); _syncTopBarLockNowBtn(); return; }
  ...
  return;
        ...
        _startTopBarSessionCountdown();
        if (!_isPinEnabled()) { unlockUI(); return; }
        if (_isSessionOpen()) { if (_pin.locked) unlockUI(); }
        return;
      }

// force lock sync across tabs
if (key === PIN_KEYS.forceLocked) {
  const v = _lsGet(PIN_KEYS.forceLocked, '0');

  if (v === '1') {
    if (_isPinEnabled()) {
      const msg = _lockReasonMessage() || '🔒 تم قفل الواجهة الآن.';
      lockUI(msg);
    }
  } else {
    // إذا انمسح في تبويب آخر (unlock)، افتح هنا أيضًا
...
async function ensureUnlockedBeforeRender() {
  ...
  if (_isForceLocked()) {
    // إذا القفل غير مفعّل/غير مضبوط، لا تترك flag عالق
    if (!_isPinEnabled()) {
      _lsDel(PIN_KEYS.forceLocked);
      _lsDel(PIN_KEYS.lockReason);
      unlockUI();
...
/* =========================
   6) شعار Splash حسب الثيم + theme-color
========================= */
function updateSplashLogo(theme) {
  ...
  const LOGO_BY_THEME = {
    default: 'default-logo-gold.png',
    elegant: 'elegant-logo-ocean-blue.png',
    corporate: 'corporate-logo-forest-green.png',
    minimal: 'minimal-logo-rose-crimson.png',
    royal: 'royal-logo-purple-gold.png',
    dark: 'dark-logo-amber-gold-glow.png'
  };
...
function initScrollButtons() {
  ...
  scrollDownBtn.addEventListener("click", () => window.scrollTo({ top: document.documentElement.scrollHeight, behavior: "smooth" }));
}
...
/* =========================
   9) رسم الواجهة (مصدر واحد)
========================= */
function redrawUI(selectedKey = Model.getSelectedKey()) {

  if (_pin?.locked) return;

  const famsRaw = Model.getFamilies();

  // طبّع إلى Object دائمًا (TreeUI غالبًا يتوقع Object)
  const fams = (famsRaw instanceof Map) ? Object.fromEntries(famsRaw.entries()) : (famsRaw || {});

  let key = selectedKey;

  const getFam = (k) => (k ? fams[k] : null);

  const isHidden = (f) => !!(f && f.hidden === true);
  const missing = (!key || !getFam(key) || isHidden(getFam(key)));

  const loaded = familiesAreLoaded();

  const firstVisibleKey = () => {
    for (const [k, f] of Object.entries(fams)) {
      if (f && !isHidden(f)) return k;
    }
    return null;
  };

  if (missing) {
    const fallback = firstVisibleKey();
    if (fallback) {
      key = fallback;

      // بعد التحميل ثبت الاختيار في model + state حتى لا يرجع يضيع
      if (loaded) {
        const curModel = Model.getSelectedKey();
        const curState = getState().selectedFamily;

        if (fallback !== curModel) Model.setSelectedKey(fallback);
        if (fallback !== curState) setState({ selectedFamily: fallback });
      }
    }
  }
...
subscribeTo(
  s => ({
    sel: s.selectedFamily,
    q: s.search,
    f: s.filters,
    uiTick: s.uiTick,
    splashHidden: s.splashHidden
  }),
  ({ sel, splashHidden }) => {
    // لا ترسم إلا بعد اختفاء السبللاش فعليًا
...
/* =========================
   11) أدوات بحث شخص داخل العائلة + فحص DEV
========================= */
...
function devAssertNoDuplicateIdsInFamily(fam, famKey) {
  ...
  walkPersonsWithPath(fam, (p, path) => {
    ...
    if (firstPathById.has(id)) {
      dups.push({ id, first: firstPathById.get(id), second: curPath });
    } else {
  ...
  if (dups.length) {
    console.error('[DEV] Duplicate _id detected', { familyKey: famKey, duplicates: dups });
    try { showWarning?.(`تحذير تطوير: يوجد تكرار في _id داخل ${highlight(famKey)}`); } catch { }
  }
}

/* =========================
   12) عمليات المستوى الأعلى (اختيار/تعديل/حذف/حفظ…)
========================= */
...
async function onDeleteFamily(key) {
  ...
  const familyLabel = (famBefore?.title || famBefore?.familyName || famBefore?.fullRootPersonName || key);
...
function onModalSave(key, familyObj) {
  ...
  const existedBefore = !!Model.getFamily(key);
...
async function onInlineRename(personId, patch) {
  ...
  // مزامنة المودال لو كان نفس الشخص
...
handlers.onHideFamily = (key) => {
  // نفّذ الإخفاء كما هو (بدون تغيير FeatureVisibility)
  FeatureVisibility.onHideFamily(key, {
    Model, redrawUI, showInfo, showSuccess, highlight, bus
  });

  // بعد الإخفاء: تأكد أن الواجهة تُحدّث فورًا
  const fams = Model.getFamilies?.() || {};
  const cur = Model.getSelectedKey?.();

  const curHidden = !!(cur && fams[cur] && fams[cur].hidden === true);

  // (1) إذا العائلة المختارة أصبحت مخفية، اختر fallback مرئي فورًا
  if (curHidden) {
    const fallback = Object.keys(fams).find(k => fams[k] && fams[k].hidden !== true) || null;

    if (fallback) {
      if (fallback !== cur) Model.setSelectedKey(fallback);
      setState({ selectedFamily: fallback, uiTick: Date.now() });
      return;
    }

    // لا يوجد أي عائلة مرئية
    setState({ selectedFamily: null, uiTick: Date.now() });
    return;
  }

  // (2) حتى لو لم تكن مختارة: اجبر rerender لتحديث الأزرار/الدروبداون
  setState({ uiTick: Date.now() });
};
...
/* =========================
   16) تقدير حجم البيانات + progress (نفس منطقك)
========================= */
...
async function progressLoadFamiliesBySize(fams) {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    const p = 30 + Math.round(ratio * 30);
...
async function bootstrap() {
  ...
  setState({ splashHidden: false });
  splashShownAt = (performance?.now?.() || Date.now());
  armSplashTimeout();
  ...
  try {
    ...
    topLockBtn?.addEventListener('click', () => bus.emit('pin:lockNow'));

    // ثيم + شعار بسرعة
    const bootTheme = getBootTheme();
    applyThemeEverywhere(bootTheme);
    ...
    // مزامنة العائلات + الفلاتر (منطق ثابت)
    ...
    window.addEventListener('FT_VISIBILITY_REFRESH', () => setState({ uiTick: Date.now() }));
    ...
    // نفس السلوك السابق: إغلاق عند النقر خارج (مرتين: bubble + capture)
    const closeFamilySwitcherIfOutside = (e) => {
    ...
    document.addEventListener('click', closeFamilySwitcherIfOutside);
    document.addEventListener('click', closeFamilySwitcherIfOutside, true);
    ...
    byId('familyButtons')?.addEventListener('click', e => {
      ...
      if (e.target.closest('.hide-family')) return;
    ...
    // (مكرر عندك سابقًا) حافظنا على نفس السلوك لكن عبر helper موحد
    applyThemeEverywhere(bootTheme);

    // تحميل إعدادات PIN من IndexedDB
    try { await PinStore.init(); } catch { }

    // تأكد من فتح/قفل PIN قبل تحميل البيانات
    await ensureUnlockedBeforeRender();
    _startTopBarSessionCountdown();
    _syncTopBarLockNowBtn();

    // ===== تحميل العائلات بعد unlock =====
    perf.start('bootstrap:loadFamilies');
    setSplashProgress(25, 'تحميل بيانات العائلات…');
    await Model.loadPersistedFamilies();
window.__familiesLoaded = true;

    bus.emit('families:coreFlag:refresh');
    setState({ selectedFamily: Model.getSelectedKey(), uiTick: Date.now() });
