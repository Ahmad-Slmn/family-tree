...
import { byId, el, textEl, showWarning, getArabicOrdinal } from '../utils.js';
...
import { buildGenerationMap, isDeceasedPerson, lifePass, describeActiveFiltersAr } from '../ui/tree.search.js';
...
const AR_TATWL = /\u0640/g;
const ALIF_ALL = /[اأإآ]/g;

const baseNorm = (s = '') =>
  String(s).normalize('NFKC').replace(AR_DIAC, '').replace(AR_TATWL, '').replace(ALIF_ALL, 'ا').trim();

// تطبيع قوي للمطابقة
const normArSearch = (s = '') => baseNorm(s).replace(/ة/g, 'ه').replace(/ى/g, 'ي');
// تطبيع للفهرسة
const normArIndex = (s = '') => baseNorm(s).replace(/ة/g, 'ه').replace(/ى/g, 'ي');
// يحترم «ة/ى» كما كتبها المستخدم
const normArQuery = (s = '') => baseNorm(s);

function getPid(p){
  const ref = p?.ref || p;
  return String(ref?._id || ref?.id || ref?.__tempId || '').trim();
}
...
function highlightInto(targetEl, name, tokensRaw){
  ...
  const tokPat = tok => {
    const core = [...tok]
      .map(ch => (/[اأإآ]/u.test(ch) ? '[اأإآ]' : esc(ch)))
      .join(AR_MARKS_OPT);
  ...
  for (const m of text.matchAll(rx)){
    ...
    targetEl.append(mark);
    last = m.index + m[0].length;
  }
  if (last < text.length) targetEl.append(text.slice(last));
}

/* ======================= ضمان جاهزية العائلة للفلاتر (pipeline + ids) ======================= */
function ensureFamilyReadyForFilters(fam){
  if (!fam || fam.__pipelineReady) return;
...
function listPersonsOfFamily(fam){
  ...
  const add = (p) => {
    ...
    const resolvedClan =
      (Lineage && typeof Lineage.resolveClan === 'function') ? (Lineage.resolveClan(p, fam, ctx) || '') : '';
    const clan = (resolvedClan || p?.bio?.clan || '').trim();

    const loose = `nr:${(p.name || '').trim()}|${(p.role || '').trim()}|${mother}|${clan}`;
    const pid = getPid(p);
    const strict = pid ? `id:${pid}` : loose;
  ...
  const tops = [...ancestors, father, root, ...wivesTop].filter(Boolean);
  ...
  tops.forEach(p => {
    ...
    if (isRoot || isTopWife) (p.children || []).forEach(add);
  });
...
function fillClanOptionsForFamily(fam){
  ...
  const { genMap } = buildGenerationMap(fam);
  ...
  persons.forEach(item => {
    ...
    // فلتر حي/متوفى
    if (!lifePass(p, filters.life)) return;
    ...
    if (filters.birthFrom || filters.birthTo){
      const by =
        (p?.bio?.birthYear != null && String(p.bio.birthYear).trim()) ? String(p.bio.birthYear).padStart(4, '0')
          : '';
    ...
    // حساب العشيرة من سياق النسب
    const resolved = (ctx && Lineage.resolveClan) ? (Lineage.resolveClan(p, fam, ctx) || '') : (b.clan || '');
...
function fillRoleOptionsForFamily(fam){
  ...
  persons.forEach(pItem => {
    ...
    // فلتر حي/متوفى
    if (!lifePass(ref, filters.life)) return;
    ...
    if (filters.clan){
      ...
      if (fc){
        const resolvedClan =
          (Lineage && typeof Lineage.resolveClan === 'function') ? (Lineage.resolveClan(ref, fam, ctx) || '') : '';

        const candidates = [
          resolvedClan,
          ref?.bio?.clan,
          ref?.bio?.motherClan,
          ref?.bio?.maternalGrandmotherClan,
          ref?.bio?.paternalGrandmotherClan
        ]
          .map(v => TreeUI.normalizeAr(String(v || '')).trim())
          .filter(Boolean);

        if (!candidates.some(pc => pc.includes(fc))) return;
      }
    }
    ...
    if (filters.birthFrom || filters.birthTo){
      const by =
        (ref?.bio?.birthYear != null && String(ref.bio.birthYear).trim()) ? String(ref.bio.birthYear).padStart(4, '0')
          : '';
    ...
    // تجميع حسب مجموعة الدور
    const g = roleGroup(ref);
    if (!g) return;
  ...
  // ترتيب حسب ROLE_FILTER_VALUES مع عدم إضافة ما لا يوجد له أشخاص
...
function fillGenerationOptionsForFamily(fam){
  const sel = byId('fltGen');
  if (!sel || !fam) return;

  const { genMap, maxGen } = buildGenerationMap(fam);
  const gens = new Set(genMap.values());

  sel.innerHTML = '<option value="">كل الأجيال</option>';

  for (let g = 0; g <= maxGen; g++){
    if (!gens.has(g)) continue;

    const opt = document.createElement('option');
    // القيمة الداخلية تبقى 0-based حتى لا تنكسر الفلترة
    opt.value = String(g);

    // العرض للمستخدم: 1-based + صياغة عربية
    const shown = g + 1;
    opt.textContent =
      (shown === 1) ? `الجيل ${getArabicOrdinal(shown)} (أعلى جد)` : `الجيل ${getArabicOrdinal(shown)}`;

    sel.appendChild(opt);
  }
}

// دالة عامة عند تغيير العائلة
export function refreshFilterOptionsForCurrentFamily(){
  const fam = getCurrentFamily();
  if (!fam) return;

  ensureFamilyReadyForFilters(fam);
  fillClanOptionsForFamily(fam);
  fillRoleOptionsForFamily(fam);
  fillGenerationOptionsForFamily(fam);

  const s = getState().filters || {};
  const roleSel = byId('fltRole');
  const clanSel = byId('fltClan');
  const lifeSel = byId('fltLife');
  const genSel = byId('fltGen');

  if (roleSel) roleSel.value = s.role || '';
  if (clanSel) clanSel.value = s.clan || '';
  if (lifeSel) lifeSel.value = s.life || '';
  if (genSel) genSel.value = (s.gen == null ? '' : String(s.gen));
}

/* ======================= ترتيب هرمي ثابت مطابق للشجرة (للبحث/الاقتراحات) ======================= */
...
function buildSuggestions(q){
  ...
  const pass = (p) => {
    ...
    // فلتر حي/متوفى
    if (!lifePass(ref, filters.life)) return false;

    // فلتر الجيل بالنسبة للـ root
    if (filters.gen != null && String(filters.gen).trim() !== ''){
      const id = getPid(ref);
      const g = genMap.get(id);
      if (g == null) return false;
      if (Number(g) !== Number(filters.gen)) return false;
    }
  ...
  return all
    ...
    .filter(p => {
      ...
      const cogRaw = String(p.cognomen || p.ref?.bio?.cognomen || '');
    ...
    .map(p => {
      const ref = p.ref || p;
      const nameNorm = normArIndex(String(ref?.name || ''));
      const roleNorm = normArIndex(String(ref?.role || ''));
      const cogNorm = normArIndex(String(ref?.bio?.cognomen || ''));
    ...
    .sort((a, b) => {
      ...
      // 2) داخل نفس المستوى: درجة المطابقة (اختياريًا)
...
function mountAdvancedFilters(){
    ...
    <div class="filters-card" role="group" aria-label="فلاتر البحث">
      ...
      <div class="filters-grid">
        ...
        <div class="form-field clan-field">
          ...
          <div class="select-wrap">
            <select id="fltClan"><option value="">كل العشائر</option></select>
          </div>
        </div>
        ...
        <div class="form-field">
          ...
          <div class="select-wrap">
            <select id="fltGen"><option value="">كل الأجيال</option></select>
          </div>
        </div>
  ...
  // تعبئة الخيارات للعائلة الحالية
  ...
  const push = () => {
    const patch = {
      filters: {
        ...
        life: byId('fltLife')?.value || '',
        gen: (byId('fltGen')?.value || '').trim(),
    ...
    localStorage.setItem('flt_life', patch.filters.life);
    localStorage.setItem('flt_gen', patch.filters.gen);

    // إعادة بناء الخيارات حسب التقاطع الجديد للفلاتر
    refreshFilterOptionsForCurrentFamily();
  };
  ...
  ['change', 'input'].forEach(ev => {
    ...
    byId('fltLife').addEventListener(ev, push);
    byId('fltGen').addEventListener(ev, push);
  });
}

function buildCantHideFiltersMessage(){
  const f = getState().filters || {};
  const activeDesc = (typeof describeActiveFiltersAr === 'function') ? describeActiveFiltersAr(f)
    : 'فلاتر مفعّلة';
  return `تعذّر إخفاء لوحة الفلاتر لوجود فلاتر نشطة (${activeDesc}). يُرجى مسح الفلاتر أولًا ثم إعادة المحاولة.`;
}
...
function wireFiltersToggle(){
  ...
  const hasActive = () => {
    const s = getState().filters || {};
    return (s.role && String(s.role).trim()) ||
      (s.clan && String(s.clan).trim()) ||
      (s.life && String(s.life).trim()) ||
      (s.gen != null && String(s.gen).trim() !== '') ||
      (s.birthFrom && String(s.birthFrom).trim()) ||
      (s.birthTo && String(s.birthTo).trim());
  };
  ...
  btn.addEventListener('click', () => {
    if (hasActive()){
showWarning(buildCantHideFiltersMessage());
      return;
    }
